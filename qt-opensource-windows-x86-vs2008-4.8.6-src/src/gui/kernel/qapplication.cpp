...
void QApplicationPrivate::process_cmdline()
{
    ...
    for (i=1; i<argc; i++) { // if you add anything here, modify QCoreApplication::arguments()
        ...
        if (!s.isEmpty()) {
            ...
            styleOverride = s;
            if (s.toLower() == "gtk+" || s.toLower() == "gtk") {
                qputenv("QT_GTKSTYLE","1");
            }
        }
    }
...
{ Q_D(QApplication); d->construct();
    QString GTKStyleRC = QString::fromLocal8Bit(qgetenv("GTK2_RC_FILES"));
    QString GTKStyleEnable = QString::fromLocal8Bit(qgetenv("QT_GTKSTYLE"));
    if (!GTKStyleRC.isEmpty() || !GTKStyleEnable.isEmpty() ) {
        setStyle("GTK+");
    } else {
        QStyleFactory::create("GTK+");
    }
}
